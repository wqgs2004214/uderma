(function($)
{
    $.fn.wScratchPad = function(option, settings)
    {
        ...
        else if(typeof option == 'string')
        {
            ...
            var elements = this.each(function()
            {
                ...
                if(data)
                {
                    ...
                    else if(option === 'update'){data.update($(this).find(":first"),settings);}
        ...
        return this.each(function()
        {
            ...
            if(!test.getContext)
            {
                elem.html("对不起，您的浏览器不支持HTML5，请升级您的浏览器！");
                return false;
            }

            var sp = new ScratchPad($settings, elem);

            elem.append($('<div></div>').css({
                position:'absolute',
                margin:0,
            ...
            elem.append($("<div id='masking'></div>").css({
                position: 'absolute',
                width:$settings.width,
                height:$settings.height,
                'z-index':'3'
    ...
    $.fn.wScratchPad.defaultSettings =
    {
        ...
        height			: 85,					// set height - best to match image height
    ...
    ScratchPad.prototype =
    {
        ...
        clear: function()
        {
            ...
            this.settings.opacity -= 10;
            if(opacity < 0){
                this.ctx.clearRect(0, 0, this.settings.width, this.settings.height);
                //showMsg(this.settings.text);
                luckPopup("wx-gcard-one");
                return;
            }
            ...
            window.setTimeout("$('#gua1').wScratchPad('clear');",20);
		},
